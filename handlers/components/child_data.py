...
from keyboards.components.child_data import get_child_data_have_passport_keyboard, get_child_data_start_keyboard
...
async def handle_child_data_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text, reply_markup=get_child_data_have_passport_keyboard(lang))
...
async def handle_child_data_get(callback: CallbackQuery, state: FSMContext):
    ...
    if have_passport == 'y':
        await state.update_data(passport_title="doc_child_stay_extension_child_data.message_2.manually.title")
    else:
        await state.update_data(passport_title="doc_child_stay_extension_child_data.message_2.manually.title", 
