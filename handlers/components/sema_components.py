...
async def start_dialogue(message: Message, state: FSMContext, callback_data=None):
    ...
    if callback_data is not None:
        ...
        if next_state is not None:
            if next_state == 'end_state':
                ...
                return
            next_state = current_state_class + ':' + current_state_btn_data['next_state']
    else:
    ...
    if 'btns' in msgs[current_state_name].keys():
        ...
        btns = {path_to_text + btn['name']:callback_start + callback_d for callback_d, btn in current_state_data['btns'].items()}
...
async def message_worked_last_year_state(event: Message | CallbackQuery, state: FSMContext, callback_data=None):
