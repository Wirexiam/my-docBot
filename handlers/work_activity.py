...
from handlers.components.passport_photo import start_new
...
async def wa_start(callback: CallbackQuery, state: FSMContext):
    """Старт: экран с описанием и кнопкой «Начать»."""
    ...
    await callback.message.edit_text(text=text, reply_markup=kbs_patent_work_activity_start(lang))


# ─────────────────────────── Паспорт: выбор способа ───────────────────────────
...
async def full_name_of_the_department(callback: CallbackQuery, state: FSMContext):
    """Запрашиваем у пользователя полное название отдела (МВД)."""
    sd = await state.get_data()
    lang = sd.get("language")
...
async def handler_full_name_of_the_department(message: Message, state: FSMContext):
    """Принимаем введённое название отдела и просим подтвердить."""
...
async def handler_passport_check(callback: CallbackQuery, state: FSMContext):
    """После подтверждения отдела — предлагаем фото/ручной ввод паспорта."""
    sd = await state.get_data()
    lang = sd.get("language")
...
@work_activity_router.callback_query(F.data == "wa_passport_manual_start")
async def wa_passport_manual_start(cb: CallbackQuery, state: FSMContext):
    """Старт ручного ввода паспорта — общий модуль, но с маршрутом WA."""
    await state.update_data(
        from_action=PatentedWorkActivity.passport_data,
        next_states=[PatentedWorkActivity.patent_entry],  # дальше — патент
...
@work_activity_router.callback_query(F.data == "wa_passport_photo_start")
async def wa_passport_photo_start(cb: CallbackQuery, state: FSMContext):
    """Старт OCR паспорта — общий модуль, но с маршрутом WA."""
    await state.update_data(
        from_action=PatentedWorkActivity.passport_data,
...
async def handle_passport_data(message: Message, state: FSMContext):
    """Получаем «кем выдан» при ручном вводе и двигаемся к патенту."""
    passport_issued = (message.text or '').strip()
    ...
    await state.update_data(passport_data=passport_data, from_action=PatentedWorkActivity.patent_entry)
...
async def handle_patient_data(message: Message, state: FSMContext):
    """Принимаем «кем выдан патент», сохраняем блок patient_data и идём к ДМС."""
    ...
    patient_data = {
        "patient_number": sd.get("patient_number"),
        "patient_date": sd.get("patient_date"),
        "patient_issue_place": sd.get("patient_issue_place"),
    }
...
async def get_name_work(message: Message, state: FSMContext):
    work_name = (message.text or '').strip()
    sd = await state.get_data()
    lang = sd.get("language")
...
async def get_INN(message: Message, state: FSMContext):
    emp_adress = (message.text or '').strip()
    sd = await state.get_data()
    lang = sd.get("language")
...
async def get_medical_policy_number(message: Message, state: FSMContext):
    """Сохраняем номер телефона → просим номер ДМС."""
    sd = await state.get_data()
    lang = sd.get("language")
    session_id = sd.get("session_id")
...
async def get_insurance_company(message: Message, state: FSMContext):
    """Сохраняем номер ДМС → просим страховую компанию."""
    sd = await state.get_data()
    lang = sd.get("language")

    policy_number = (message.text or '').strip()
...
async def get_medical_policy_validity_period(message: Message, state: FSMContext):
    """Сохраняем страховую компанию → просим срок действия ДМС."""
    sd = await state.get_data()
    lang = sd.get("language")

    medical_policy_company = (message.text or '').strip()
...
async def get_medical_policy_polis_date(message: Message, state: FSMContext):
    """Сохраняем срок действия полиса → экран подтверждения всей анкеты."""
    sd = await state.get_data()
    lang = sd.get("language")

    current_state = await state.get_state()
    if current_state == PatentedWorkActivity.medical_policy_polis_date.state:
        medical_policy_polis_date = (message.text or '').strip()
    ...
    text = (
        ...
        f"{passport.get('passport_serial_number')}, {passport.get('passport_issue_date')}, "
        f"{passport.get('passport_issued')}, {passport.get('passport_expiry_date')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.patent', lang)}: "
        f"{patient_data.get('patient_number')}, {patient_data.get('patient_date')}, {sd.get('patient_issue_place')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.work_adress', lang)}: {sd.get('emp_adress')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.profession', lang)}: {sd.get('work_name')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.inn', lang)}: {sd.get('inn')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.policy', lang)}: "
        f"{sd.get('policy_number')}, {sd.get('medical_policy_company')}, {sd.get('medical_policy_polis_date')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.phone_number', lang)}: {sd.get('phone_number')}"
    )
    await message.answer(text=text, reply_markup=kbs_policy_data_confirmation(lang))


# ─────────────────────────── Редакторы (меню «изменить») ───────────────────────────
...
async def apply_edit_passport_field(message: Message, state: FSMContext):
    """Применяем правку в passport_data."""
    sd = await state.get_data()
    field = sd.get("edit_passport_field")
    passport = dict(sd.get("passport_data") or {})
    passport[field] = (message.text or '').strip()
...
async def apply_edit_patent_field(message: Message, state: FSMContext):
    """Применяем правку в patient_data (раскидано по state, как в исходнике)."""
    sd = await state.get_data()
    field = sd.get("edit_patent_fields")
    await state.update_data({field: (message.text or '').strip()}, edit_patent_fields=None)
    await get_medical_policy_polis_date(message, state)


# ───────────────────────── Принято → сохраняем и генерим ─────────────────────────

def to_uppercase(data: dict) -> dict:
    result = {}
    for k, v in data.items():
        if isinstance(v, dict):
            result[k] = to_uppercase(v)
...
async def accept_and_generate_wa(query: CallbackQuery, state: FSMContext):
    """
    ЕДИНСТВЕННЫЙ обработчик подтверждения:
    1) сохраняет данные в SDM,
    2) генерирует DOCX и отправляет пользователю.
    """
    sd = await state.get_data()
    session_id = sd.get("session_id")
    lang = sd.get("language")
    ...
    user_data = {
        "medical_policy": {
            "policy_number": sd.get("policy_number"),
            "medical_policy_company": sd.get("medical_policy_company"),
            "medical_policy_polis_date": sd.get("medical_policy_polis_date"),
        },
        "inn": sd.get("inn"),
    ...
    # 2) Генерация DOCX
    mvd_adress = sd.get("department_full_name", "")
    mvd_adress_1 = mvd_adress[:31] if mvd_adress else ""
    mvd_adress_2 = mvd_adress[31:31*2] if mvd_adress else ""
    mvd_adress_3 = mvd_adress[31*2:31*3] if mvd_adress else ""
    ...
    issue_date = passport_data.get("passport_issue_date", "")
    idp = issue_date.split(".") if issue_date else []
    passport_issue_date_day = idp[0] if len(idp) > 0 else ""
    passport_issue_date_month = idp[1] if len(idp) > 1 else ""
    passport_issue_date_year = idp[2] if len(idp) > 2 else ""

    passport_issue_place = passport_data.get("passport_issued", "") or ""
    passport_issue_place_1 = passport_issue_place[:25] if passport_issue_place else ""
    passport_issue_place_2 = passport_issue_place[25:25*2+1] if passport_issue_place else ""

    patient_data = sd.get("patient_data", {}) or {}
    patent_number = (patient_data.get("patient_number") or "")
    patent_series = patent_number[:2]
    patent_numbers = patent_number[2:].replace("-", "").replace(" ", "")

    patient_date = patient_data.get("patient_date", "")
    pd = patient_date.split(".") if patient_date else []
    patent_issue_date_day = pd[0] if len(pd) > 0 else ""
    patent_issue_date_month = pd[1] if len(pd) > 1 else ""
    patent_issue_date_year = pd[2] if len(pd) > 2 else ""

    job_name = sd.get("work_name", "") or ""
    job_name_1 = job_name[:31]
    job_name_2 = job_name[31:31*2]
    job_name_3 = job_name[31*2:31*3]

    work_adress = sd.get("emp_adress", "") or ""
    work_adress_1 = work_adress[:31]
    work_adress_2 = work_adress[31:31*2]

    med_data_name = sd.get("medical_policy_company", "") or ""
    med_data_name_1 = med_data_name[:32]
    med_data_name_2 = med_data_name[32:32*2]
    med_data_name_3 = med_data_name[32*2:32*3]

    med_policy_date = sd.get("medical_policy_polis_date", "") or ""
    if " по " in med_policy_date:
        start, _end = med_policy_date.split(" по ", 1)
    ...
    med_policy_series_number = sd.get("policy_number", "") or ""
    ...
    context = {
        ...
        "char_phone": sd.get("phone_number", ""),
    }
    context = to_uppercase(context)
    ...
    await state.clear()
    await query.message.edit_text(f"{_.get_text('ready_to_download_doc', lang)}\n")
