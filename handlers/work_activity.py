...
async def handler_passport_check(callback: CallbackQuery, state: FSMContext):
    """Получаем положительный ответ и запращиваем способ ввода паспорта"""
    state_data = await state.get_data()
    lang = state_data.get("language")
...
async def handle_passport_data(message: Message, state: FSMContext):
    """
    Этот хендлер нужен только когда паспорт вводили ВРУЧНУЮ:
    тут прилетает последний ответ «кем выдан», после чего сразу начинаем патент.
    """
    passport_issued = message.text.strip()
    passport_data = (await state.get_data()).get("passport_data", {}) or {}
...
async def handle_patient_data(message: Message, state: FSMContext):
    """Сохраняем данные патента и запрашиваем ДМС"""
...
async def get_medical_policy_polis_date(message: Message, state: FSMContext):
    ...
    patient_data = sd.get("patient_data", {})
    pprint(sd)
    text = (
        f"{_.get_text('wa_patent.edit_wa_data.title', lang)}\n\n"
        f"{_.get_text('wa_patent.edit_wa_data.full_name', lang)}: {passport.get('full_name')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.passport', lang)}: {passport.get('passport_serial_number')}, "
        f"{passport.get('passport_issue_date')}, {passport.get('passport_issued')}, {passport.get('passport_expiry_date')}\n"
        f"{_.get_text('wa_patent.edit_wa_data.patent', lang)}: {patient_data.get('patient_number')}, "
        f"{patient_data.get('patient_date')}, {sd.get('patient_issue_place')}\n"
        ...
        f"{_.get_text('wa_patent.edit_wa_data.policy', lang)}: {sd.get('policy_number')}, "
        f"{sd.get('medical_policy_company')}, {sd.get('medical_policy_polis_date')}\n"
...
async def handle_all_true_in_wa(callback: CallbackQuery, state: FSMContext):
    """Обработка подтверждения правильности данных перед генерацией документа"""
    state_data = await state.get_data()
...
async def edit_passport_fields(message: Message, state: FSMContext):
    ...
    await state.update_data(passport=passport, edit_passport_field=None)
    await get_medical_policy_polis_date(message, state)
...
async def edit_patent_fields(message: Message, state: FSMContext):
    sd = await state.get_data()
