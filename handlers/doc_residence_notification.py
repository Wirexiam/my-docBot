...
async def handle_travel_outside_Ru_place(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=text, reply_markup=get_travel_outside_Ru_check_keyboard(lang)
    )
...
async def end_worked_last_year(message: Message, state: FSMContext, edit_msg=None):
    ...
    info = (
        [
            ...
            for work in data["worked_last_year"]
        ]
...
async def handle_income_last_year_start(callback: CallbackQuery, state: FSMContext):
    ...
    if callback_data == "btn_back":
        ...
        if is_now_edit:
            ...
            await callback.message.edit_text(
                text, reply_markup=get_callback_btns(btns, lang)
            )
...
async def handle_income_last_year_income(message: Message, state: FSMContext):
    ...
    info = (
        ...
        if "income_last_year" in state_data.keys()
        else []
    )
    ...
    text = (
        f"{_.get_text('income_last_year.message_3.title', lang)}\n\n{'\n\n'.join(info)}"
    )
...
async def handle_income_last_year_after_income(
    callback: CallbackQuery, state: FSMContext
):
...
async def check_doc_residence_notification(
    ...
    income_last_year_str = "\n".join(
        [
            (
                f"{_.get_text(f'income_last_year.message_1.{data["form_NDFL"]}', lang)}, {data["income"]}"
                if data["income"] != 0
