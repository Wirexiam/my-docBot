...
def parse_ru_address(addr: str) -> dict:
    ...
    m_street = re.search(r"(ул\.?|улица|просп\.?|проспект|шоссе|пер\.?|переулок|наб\.?|набережная|пр-д|проезд)\s+(.*)$", street_raw, re.IGNORECASE)
    if m_street:
    ...
    return {
        ...
        "char_street_name": street_name_2.split(" ")[0] if street_name_2 else "",  # если в шаблоне нужно одно слово
        "house_adress": house,
        "corpus": corpus,
        "room": flat,
    }
...
async def arrival_to_kid_agree(callback: CallbackQuery, state: FSMContext):
    ...
    async def arrival_passport_photo_alias(cb: CallbackQuery, state: FSMContext):
        ...
        """
        from handlers.components.passport_photo import start_new  # локальный импорт, чтобы не плодить циклов
        await state.update_data(from_action=Arrival_transfer.after_passport, ocr_flow="arrival")
...
async def arrival_migr_card(message: Message, state: FSMContext):
    ...
    next_states = [HomeMigrData.adress, Arrival_transfer.after_about_home]
    await state.update_data(next_states=next_states, from_action=Arrival_transfer.after_about_home)
...
async def true_arrival_doc(event: CallbackQuery, state: FSMContext):
    ...
    data = {
        ...
        "char_arrive_date_day":  (sd.get("migration_data", {}).get("entry_date", "  .  .    ").split("."))[0],
        "char_arrive_date_month":(sd.get("migration_data", {}).get("entry_date", "  .  .    ").split("."))[1],
        "char_arrive_date_year": (sd.get("migration_data", {}).get("entry_date", "  .  .    ").split("."))[2],
        "char_exit_date_day":    (sd.get("migration_data", {}).get("pretria_period", "  .  .    ").split("."))[0],
        "char_exit_date_month":  (sd.get("migration_data", {}).get("pretria_period", "  .  .    ").split("."))[1],
        "char_exit_date_year":   (sd.get("migration_data", {}).get("pretria_period", "  .  .    ").split("."))[2],
        "char_migr_cart_series": (sd.get("migration_data", {}).get("number_migr_card_arrival", " ").split(" "))[0],
        "char_migr_cart_numbers":(" ".join(sd.get("migration_data", {}).get("number_migr_card_arrival", " ").split(" ")[1:])).strip(),
    ...
    if sd.get("passport_data"):
        ...
        data.update({
            ...
            "char_birth_date_day":  (pd.get("birth_date", "  .  .    ").split("."))[0],
            "char_birth_date_month":(pd.get("birth_date", "  .  .    ").split("."))[1],
            "char_birth_date_year": (pd.get("birth_date", "  .  .    ").split("."))[2],
            "char_passport_numbers": pd.get("passport_serial_number", ""),
            "char_passport_issue_date_day":  (pd.get("passport_issue_date", "  .  .    ").split("."))[0],
            "char_passport_issue_date_month":(pd.get("passport_issue_date", "  .  .    ").split("."))[1],
            "char_passport_issue_date_year": (pd.get("passport_issue_date", "  .  .    ").split("."))[2],
            "char_passport_expire_date_day":  (pd.get("passport_expiry_date", "  .  .    ").split("."))[0],
            "char_passport_expire_date_month":(pd.get("passport_expiry_date", "  .  .    ").split("."))[1],
            "char_passport_expire_date_year": (pd.get("passport_expiry_date", "  .  .    ").split("."))[2],
        })
    else:
        ...
        data.update({
            ...
            "char_birth_date_day":  (cd.get("birth_date", "  .  .    ").split("."))[0],
            "char_birth_date_month":(cd.get("birth_date", "  .  .    ").split("."))[1],
            "char_birth_date_year": (cd.get("birth_date", "  .  .    ").split("."))[2],
    ...
    if sd.get("organization_data"):
        ...
        def oget(i): return oparts[i] if i < len(oparts) else ""
        ...
        data.update({
            ...
            "char_phone":                 (sd.get("phone_by_organisation", "")[1:] if sd.get("phone_by_organisation") else ""),
        })
    else:
        ...
        if ind:
            ...
            data.update({
                ...
                "char_reciever_passport_series": (ind.get("passport_serial_number_input", "").split(" ") + [""])[0],
                "char_reciever_passport_numbers": " ".join((ind.get("passport_serial_number_input", "").split(" "))[1:]),
                "char_reciever_passport_issue_date_day":   (ind.get("passport_give_date_input", "  .  .    ").split("."))[0],
                "char_reciever_passport_issue_date_month": (ind.get("passport_give_date_input", "  .  .    ").split("."))[1],
                "char_reciever_passport_issue_date_year":  (ind.get("passport_give_date_input", "  .  .    ").split("."))[2],
